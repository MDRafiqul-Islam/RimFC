@extends('manager.welcome')
@section('content')

<br>

<table id="players">
    @foreach ($fixture as $fixture)
    <tr>
      <td >{{$fixture->date}}</td>
      <td> <img style="border-radius: 50%; " width="56px;" src="{{url('RIMFC.jpg')}}"></td>
      <td >RIMFC</td>
      <td >{{$fixture->time}}</td>
      <td >{{$fixture->opponent}}</td>
      <td>
          <img style="border-radius: 50%; " width="56px;" src=" {{asset('storage/fixture/'.$fixture->photo)}}" alt="players">

      </td>
      <td>
          @if ($fixture->formationstatus != 1)
            <a href="{{ route('manager.pages.formation', $fixture->id )}}"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-person-fill" viewBox="0 0 16 16">
                <path d="M12 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zm-1 7a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm-3 4c2.623 0 4.146.826 5 1.755V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1v-1.245C3.854 11.825 5.377 11 8 11z"/>
              </svg></a>
              @endif
      </td>
    </tr>
    @endforeach
</table>
<br>

@endsection
