...
<div class="sec">
    <div class="img">
    <img src="{{ url('storage/formation/4-1-2-1-2.jpg') }}">
</div>
    ...
    <form action="{{ route('manager.pages.createFormation') }}" method="POST">
        ...
        <td><input type="hidden" name="formation[]" value="4-1-2-1-2.jpg"></td>
        <td><select name="name[]"><option>Select Player</option>@foreach ($players as $key=>$player) @if ($player->available == 'yes')<option value="{{$player->id}}">{{$player->name}}</option>@endif @endforeach</select></td>
